...
def get_CIFAR10_data(num_training=49000, num_validation=1000, num_test=1000,
    ...
    cifar10_dir = '/home/annus/Desktop/Pyhon Projects/cs231nprojects/neural network/Cifar10data'
...
Y = np.zeros((m,10)) # ==> m*K = m*10
Y[np.arange(m),y] = 1

Yval = np.zeros((mval,10)) # ==> m*K = m*10
Yval[np.arange(mval),yval] = 1


"""accuracy = np.mean(np.argmax(Y,axis=1) == y)
print('accuracy = ', accuracy*100,'%\n')

accuracy = np.mean(np.argmax(Yval,axis=1) == yval)
print('accuracy = ', accuracy*100,'%\n')"""

# new data for learning curves
'''X_real = X
y_real = y
Xval_real =Xval
yval_real = yval
